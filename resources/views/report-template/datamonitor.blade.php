...
<body>
		...
		if($data == null){
			...
			}else if(isset($no_data) && $no_data == 'true'){
				echo "<div class='alert alert-danger text-center' role='alert'>
	...
	<div class="wrap" >
        <div>
			...
			<label class='control-label top-label'>မြို့နယ်<span class='top-label-value'>{{ $township }}</span></label>

			@if(isset($reported_hf_count))

			<label class="control-label top-label">ပုံစံပေးပို့သောကျန်းမာရေးဌာနစုစုပေါင်းအရေအတွက် - <span class="top-label-value">{{ $reported_hf_count[0]->sum }}</span></label>

			<label class="control-label top-label">ပေးပို့သောစုစုပေါင်းအရေအတွက် - <span class="top-label-value">{{ count($data) }}</span></label>
			
			<label class="control-label top-label">မြို့နယ်ရှိကျန်းမာရေးဌာနစုစုပေါင်း - <span class="top-label-value">{{ $total_hfm }}</span></label>
	...
	<script>
		$( document ).ready(function() {		
			...
			for (var i = 1; i<2;  i++) {
				...
				if(parseInt(col.innerHTML) || col.innerHTML == "0" && col.headers != "PYear" && col.headers.toLowerCase() != "pyear" 
				&& col.headers.toLowerCase() != "year" && col.headers.toLowerCase() != "monthcode"
				&& col.headers != 'CQ' && col.headers != 'PQ' && col.headers != 'Form Code' && col.headers != 'Row No'){
					var totalVal = 0;
					...
					if($("#tf_"+j).html() == "PYear" || $("#tf_"+j).html() == "PMonth" || $("#tf_"+j).html() == "Agegroup" || $('#tf_' + j).html() == "Year"
					|| $("#tf_"+j).html() == "CQ" || $("#tf_"+j).html() == "PQ" || $("#tf_"+j).html() == "Form_code" || $("#tf_"+j).html() === "Row_No"
					|| $("#tf_"+j).html() === "Row_no" || $('#tf_' + j).html() == 'PQ_Given' || $('#tf_'+j).html() == "Pt_Location"
					|| $('#tf_'+j).html() == "Referral" || $('#tf_'+j).html() == "Age_Year" || $('#tf_'+j).html() == "age_year" || $('#tf_'+j).html() == "Form_No"
					|| $('#tf_'+j).html() == "Screening_Date" || $('#tf_'+j).html() == "Row_count" || $('#tf_'+j).html() == "Form_Code"
					|| $('#tf_'+j).html() == "Program_Date" || $('#tf_'+j).html() == "Data_Entry_Date" || $('#tf_'+j).html() == "Month_Delayed" || $('#tf_'+j).html() == "PYear:1"
					|| $('#tf_'+j).html() == "No"){
						$("#tf_"+j).html("");
					} else {
						$("#tf_"+j).html(totalVal);
