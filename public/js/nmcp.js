...
function isValidDate(d, m, y) {
    ...
    if (parseInt(d) && parseInt(m) && parseInt(y)) {
        if (parseInt(d) === 9 && parseInt(m) === 9 && parseInt(y) === 999) {
            ...
            if (parseInt(d) > 31 || parseInt(m) > 12 || parseInt(y) < 1900 || parseInt(y) > 9999) {
                return false;
            }
    ...
    if (sr_code != "0" && sr_code.length == 6) {
        try {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("." + ts_id).append("<option value='0' selected> All Township </option>");
                    ...
                    jQuery.each(data, function(i, val) {
                        var opt = "<option value='" + val.ts_code + "'>" + val.ts_name + "</option>";
                        $("." + ts_id).append(opt);
                    });
        ...
        try {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $("#" + ts_id).html("");
                    $("#" + ts_id).append("<option value='0'> ရွေးရန် </option>");
                    $("#" + ts_id).prop("disabled", false);
                    jQuery.each(data, function(i, val) {
                        var opt = "<option value='" + val.ts_code + "'>" + val.ts_name + "</option>";
                        $("#" + ts_id).append(opt);
        ...
        $("." + ts_id).html("");
        $("." + ts_id).append("<option value='0' selected> မြို့နယ်အားလုံး </option>");
    }
}
    ...
    try {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#" + target_control_id).append("<option value='reportby_rhc'> ရွေးရန် </option>");
                $("#" + target_control_id).prop("disabled", false);
    ...
    try {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                jQuery.each(data, function(i, val) {
                    var opt = "<option value='" + val.d_code + "'>" + val.d_name + " | " + val.d_name_mmr + "</option>";
    ...
    try {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#" + target_control_id).append("<option value='0' selected disabled> ရွေးရန် </option>");
                $("#" + target_control_id).prop("disabled", false);
...
function delete_data_entry_form() {
    ...
    bootbox.confirm('ဖောင်နှင့်သက်ဆိုင်သောဒေတာများအားအပြီးဖျက်မည်။ သေချာပါက OK နှိပ်ပါ။', function(result) {
        if (result == false) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data == "1") {
                            ...
                            bootbox.alert("အောက်ပါအချက်အလက်များနှင့်ကိုက်ညီသော ဖောင်မရှိပါ");
                            reset_button_loading('btn_delete_data_entry_form', btn_text);
                        } else {
                            bootbox.alert(data);
                        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                jQuery.each(data, function(i, val) {
                    ...
                    $("#last_corefacility_container").append('<div class="form-group" id="de-date">' + val.DE_DateTime + '</div>');
                    //console.log('mzz', val);
                    $("#last_corefacility_container").append('<div class="form-group" style="text-align:center;"><a href="forms">အချက်အလက်အားလုံးကြည့်ရန်</a></div>');
                });
            }
        });
    } catch (err) {
    ...
    if ((char >= 0) && (char <= 110)) {
        if (char <= 10 || char >= 60) {
            if (sex == 1 && preg == 1) {
                ...
                tr.find('.preg').val(tr.find('.preg').val() == '' ? '' : '0').trigger('change').css('cursor', 'not-allowed').prop('disabled', true);
            } else {
                tr.find('.preg').val(tr.find('.preg').val() == '' ? '' : '0').trigger('change').css('cursor', 'not-allowed').prop('disabled', true);
            }
        } else {
            tr.find('td:eq(4) select').focus();
            tr.find('.preg').val(tr.find('.preg').val() == '0' ? '0' : '').trigger('change').css('cursor', 'default').prop('disabled', false);
        }
    } else {
        if (char == 999) {
            tr.find('td:eq(4) select').focus();
            ...
            bootbox.alert("အသက် ၁၁၀ အောက်ဖြစ်ရမည်။ (သို့) ၉၉၉ ဟုရိုက်ပါ။").on('hidden.bs.modal', function() {
                // $(age).focus();
                $(age).val('');
            });
            // alert("အသက် ၁၁၀ အောက်ဖြစ်ရမည်။ (သို့) ၉၉၉ ဟုရိုက်ပါ။");
            // tr.find('input.age').select();
            // tr.find('input.age').val('');
        }
    }
}
...
function checkSex(data) {
    ...
    if (data.value == '0') {
        ...
        // tr.find('td:eq(8) select').focus();
        tr.find('td:eq(7) select').val(7).trigger('change').prop('disabled', true).css('cursor', 'not-allowed');
    } else {
        var age = tr.find('td:eq(3) input').val();
        if (age < 10 || age > 60) {
            // bootbox.alert("အသက် ၁၀ နှစ်နှင့် ၆၀ အတွင်းသာ \"ကိုယ်ဝန်ဆောင်\" ရွေးခွင့်ရှိသည်။").on('hidden.bs.modal', function(){
            //     $(data).focus();
            // });
            ...
            //alert("Age between 10 year and 60 year can choose preg field");
            // tr.find('td:eq(7) select').prop('selectedIndex', 0);
            // tr.find('td:eq(7) select').prop('disabled', false);
            // tr.find('td:eq(7) select').css('cursor', 'default');
            // tr.find('td:eq(7) select').focus();
...
//     var rtd = tr.find('td:eq(10) select').val();
//     var micro = tr.find('td:eq(9) select').val();
//     var result;
...
// var rtd = 2; // Replace with your desired values
// var micro = 1; // Replace with your desired values

// var result = getAssumResult(rtdValue, microValue);
...
function checkMpdeath(mmm) {
    ...
    if (mmm.value == '1') {
        if ((rcs == '0' || rcs == '7' || rcs == '9') && (rdt == '0' || rdt == '7' || rdt == '9')) {
            ...
            bootbox.alert("ငှက်ဖျားပိုးကြောင့်သေဆုံးခြင်းသေချာမှရွေးပါ").on('hidden.bs.modal', () => {
                $(mmm).focus();
            });
        }
    }
            ...
            case '1':
                if (!(positive_value.indexOf(parseInt(rdt_value)) !== -1 ||
                    ...
                    bootbox.alert('ငှက်ဖျားပိုးမရှိပါ။').on('hidden.bs.modal', function() {
                        ...
                        positive_value.indexOf(parseInt(rcs_value)) === 1)) {
                    bootbox.alert('Pv/Non Pf ပိုးတွင်ဖယ်လ်ဆီပရမ်မပေးပါ။').on('hidden.bs.modal', function() {
                        $(act).focus();
                    });
        ...
        if (this.className === 'cq only-integer') {
            ...
            if (parseInt(cq) > 50) {
                ...
                if (parseInt(rcs_value) == 1 || parseInt(rdt_value) == 1 || parseInt(rcs_value) == 3 || parseInt(rdt_value) == 3) {
                    bootbox.alert('Pf ပိုးတွင် CQ မပေးပါ။(N/A for 77 or Missing for 99)').on('hidden.bs.modal', function() {
                        $(cqpq).focus();
                    });
                }
                if ((parseInt(cq) == 0) &&
    ...
    $('.cqvhv, .pqvhv').on('change', function(e) {
        ...
        if (this.className === 'pqvhv only-integer') {
            ...
            if (parseInt(pq) > 50) {
                ...
                if (parseInt(age) < 1 || parseInt(preg) == 1) {
                    bootbox.alert('အသက်(၁)နှစ်အောက်နှင့်ကိုယ်ဝန်ဆောင်များကို PQ မပေးပါ။').on('hidden.bs.modal', function() {
                        $(cqvhv).focus();
                    });
                }
                if ((positive_value.indexOf(parseInt(rcs_value)) != -1 || positive_value.indexOf(parseInt(rdt_value)) != -1) &&
